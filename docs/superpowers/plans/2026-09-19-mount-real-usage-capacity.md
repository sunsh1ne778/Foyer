...
- **逻辑用量 vs 物理占用**：`summaries[].size` 是元数据逻辑大小（`align4K(文件长度)` 累加），「仅导入元数据」时它统计的是宿主盘上已存在的文件，与 JuiceFS 实际占的对象存储空间无关。2026-09-19 实测：`/av_20260619` = 537 GiB 逻辑，全卷逻辑 2266 GiB，**而物理盘只用了 73.8 GiB / 1007 GiB（7.3%）**。
  进度条分母取物理盘总量后，百分比读作「该挂载的逻辑数据相对物理盘的比例」，不是物理占用率 —— **其后果是卷挂载的进度条被算成 100%**（2266 GiB ÷ 1006 GiB = 225%，钳到 100），而真实磁盘占用只有 7.3%。这不是「进度条反应真实磁盘占用」。
  分子分母是两个不同量纲（元数据逻辑大小 vs 物理盘容量），混在一个比值里只有在「逻辑 ≈ 物理」时才有意义；仅导入元数据时该前提不成立。`volume.disk_used/disk_free` 单独暴露了物理真值，UI 若要有「真实磁盘占用」条应改用 `disk_used / disk_total`。**待用户决定进度条语义（见下）。**
